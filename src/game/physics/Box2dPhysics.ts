import { IPhysics, MapEntity, MapEntityState } from '../types/IPhysics';

// Dynamic import for browser-only loading
async function loadBox2D() {
  if (typeof window === 'undefined') {
    throw new Error('Box2D can only be loaded in browser environment');
  }
  const Box2DModule = await import('box2d-wasm');
  return Box2DModule.default;
}
...
export class Box2dPhysics implements IPhysics {
  ...
  async init(): Promise<void> {
    const Box2DFactory = await loadBox2D();
      ...
      switch (entity.shape.type) {
        case 'box':
          ...
          shape.SetAsBox(
            ...
            entity.shape.height!,
            0,
            entity.shape.rotation || 0,
          );
          fixtureDef.set_shape(shape);
          body.CreateFixture(fixtureDef);
          break;
        case 'polyline':
          if (entity.shape.points && entity.shape.points.length > 1) {
